...
namespace StudyHub.Controllers
{
    ...
    [ApiController]
    public class CacXuLyLamBai : ControllerBase
    {
        ...
        public IActionResult TinhDiemBaiLam(int idSinhVien, [FromBody] List<TraloiSinhVienDTO> traloiSinhVien)
        {
            ...
            foreach (var cauhoi  in traloiSinhVien)
            {
                ...
                var cauHoi = _cauHoiBLL.GetCauHoiByIdDapAn(cauhoi.IdDapan);
            ...
            var cauHoiCheck = _cauHoiBLL.GetCauHoiByIdDapAn(traloiSinhVien.FirstOrDefault().IdDapan);
            var cotDiem = new CotDiem
            {
        ...
        [HttpPost("xuatketquahoctaptheoki")]
        public IActionResult GetKetQuaHocTapbyKi([FromBody] SinhVienKyHocDto sinhVienKyHocDto)
        {
            var bangDiemtheoKy = new BangDiemtheoKy();
            var sinhvien = _userBLL.GetUserById(sinhVienKyHocDto.IdSinhVien);
            ...
            bangDiemtheoKy.NamHocKy = _khoaHocBLL.GetHocKyById(sinhVienKyHocDto.IdKyHoc).NamHocKy;
            foreach (var khoahoc in khoahhocdangkis)
            {
                ...
                var ketquahocTap = new KetQuaKhoaHoc()
                {
                    ...
                    TenKhoaHoc = _khoaHocBLL.GetKhoaHocById(khoahoc.IdKhoaHoc).TenKhoaHoc,
                    Diems = diemList.Select(d => new DiemofsinhvienDto
                    {
                        Tencotdiem = d.TenCotDiem,
                        Diem = d.Diem
